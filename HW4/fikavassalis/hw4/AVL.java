...
public class AVL<Key extends Comparable<Key>> {
	...
	public AVL(Key[] ordered) {
		/** FILL IN HERE. **/
		this.treeArray = new Comparable[ordered.length];
		
		for(int i=1; i<=((Math.log(ordered.length) + 1) / Math.log(2)); i++) {
			this.retrieveGivenLevel(createNoRotations(ordered, 0, ordered.length - 1), i);
		}
		
		for(int i=0; i<treeArray.length; i++) {
			this.insert((Key) treeArray[i]);
		}
		
		StdOut.println ("number of rotations:" + rotations);
	}
	
	void retrieveGivenLevel(Node root, int level)
	{
		if (root == null)
			return;
		if (level == 1) {
			treeArray[treeArrayIndex++] = root.key;
		} else if (level > 1) {
			retrieveGivenLevel(root.left, level-1);
			retrieveGivenLevel(root.right, level-1);
		}
	}
	
	public Node createNoRotations(Key[] arr, int start, int end) {
		if (start > end) {
            return null;
        }
    	...
    	node.right = createNoRotations(arr, mid + 1, end);
    	node.left = createNoRotations(arr, start, mid - 1);
	...
	public void merge(AVL<Key> other) {
		/** FILL IN HERE. **/
		Queue<Key> keysOther = other.keys();
		
		while(!keysOther.isEmpty()) {
			this.insert(keysOther.dequeue());
		}
	}
