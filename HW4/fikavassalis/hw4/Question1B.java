...
public class Question1B {
	...
	public static void main(String[] args) {
		...
		// Fill in Here. 
		Queue<Integer> oneCopyValues = oneCopy.keys();
		Queue<Integer> twoValues = two.keys();
		while(!oneCopyValues.isEmpty()) {
			Integer val = oneCopyValues.dequeue();
		...
		while(!twoValues.isEmpty()) {
			Integer val = twoValues.dequeue();
			if(table.contains(val)) {
				table.put(val, table.get(val) + 1);
			} else {
				table.put(val, 1);
		...
		// Reduce occurrences by one, every time a value is found on "one".
		Queue<Integer> oneValues = one.keys();
		while(!oneValues.isEmpty()) {
			Integer val = oneValues.dequeue();
			table.delete(val);
		}
