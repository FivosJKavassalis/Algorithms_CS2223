...
public class BreadthFirstSearch {
    ...
    public Iterable<Integer> pathTo(int v) {
        ...
        int x;
        for (x = v; getDistTo()[x] != 0; x = edgeTo[x])
            path.push(x);
        path.push(x);
        return path;
    }
    ...
    private boolean check(Graph G, int s) {
        ...
        if (getDistTo()[s] != 0) {
            StdOut.println("distance of source " + s + " to itself = " + getDistTo()[s]);
            return false;
        }
        ...
        for (int v = 0; v < G.V(); v++) {
            for (int w : G.adj(v)) {
                ...
                if (hasPathTo(v) && (getDistTo()[w] > getDistTo()[v] + 1)) {
                    StdOut.println("edge " + v + "-" + w);
                    StdOut.println("distTo[" + v + "] = " + getDistTo()[v]);
                    StdOut.println("distTo[" + w + "] = " + getDistTo()[w]);
                    return false;
                }
            }
        }
